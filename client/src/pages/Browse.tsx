...
const Details = (props: Event) => {
  ...
  const toStringTime = (t: Interval): string => {
    ...
    const times = [
      t.startHr === 0 ? 12 : t.startHr > 12 ? t.startHr % 12 : t.startHr,
      t.startMin,
      t.endHr === 0 ? 12 : t.endHr > 12 ? t.endHr % 12 : t.endHr,
      t.endMin,
    ].map((x) => (x < 10 ? `0${x}` : `${x}`));
...
const MultiFilter = (
    ...
    changeExpand: (p: string | false, e: any) => void;
  ...
  const changeActivity = (f: string, e: any) => {
    e.target.checked
      ? props.setFilters(filters.concat([f]))
      : props.setFilters(filters.filter((x) => x !== f));
  };

  return (
    <>
      <Accordion
        expanded={panel === props.accordionName}
        onChange={(e) => props.changeExpand(props.accordionName, e)}
      >
        ...
        <AccordionDetails sx={{ backgroundColor: lightGrayBlue }}>
          <FormGroup>
            {props.labelNames.map((item: string) => (
              <FormControlLabel
                key={item}
...
const BrowsePage = () => {
  ...
  const timeToInteral = (t: string): Interval =>
    t === "Morning"
      ? { startHr: 6, startMin: 0, endHr: 12, endMin: 0 }
      : t === "Afternoon"
      ? { startHr: 12, startMin: 0, endHr: 18, endMin: 0 }
  ...
  const filteredEvents: Event[] = useMemo(() => {
    const nonJoinedEvents = searchedEvents.filter(
      (e) => !e.users.includes(netid ?? "")
    );

    if (filters.length === 0) return nonJoinedEvents;

    let dayFilters: Day[] = filters
      .filter((item) => isDay(item))
      .map((d) => d.substring(0, 2)) as Day[];
    dayFilters =
      dayFilters.length == 0
        ? (allDaysOfWeek.map((day) => day.substring(0, 2)) as Day[])
        : dayFilters;

    const timeFilters: string[] = filters.filter((item) => isTime(item));

    const dayFiltered = nonJoinedEvents.filter((e) =>
      (e.days.length == 0 ? [numToDay(new Date().getDay())] : e.days).some(
        (d) => dayFilters.includes(d)
      )
    );

    return timeFilters.length == 0
      ? dayFiltered
      : dayFiltered.filter((e) =>
          timeFilters.some((time) => overlapTime(timeToInteral(time), e.time))
        );
  ...
  const currentEvents = useMemo(() => {
    ...
    return nowEvents.length == 0 ? (
      ...
      nowEvents.map((item) => (
        <EventListing {...item} key={item.title + item.host} />
      ))
    );
  }, [filteredEvents]);
        ...
        <LoginPrompt loggedIn={loggedIn}>
          <Stack direction="row">
            ...
            <Container sx={{ width: 1 / 4 }}>
              ...
              <Paper
                elevation={1}
                sx={{ padding: 1, backgroundColor: grayBlue }}
              >
                <Typography variant="h4">
                  {" "}
                  <div style={{ fontWeight: 600 }}>Current Filters:</div>{" "}
                  {filters.length == 0 ? "None" : filters.join(", ")}
                </Typography>
              </Paper>
